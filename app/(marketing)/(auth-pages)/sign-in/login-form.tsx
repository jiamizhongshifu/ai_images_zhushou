...
export default function LoginForm({ message }: LoginFormProps) {
  ...
  // 使用Google登录
  const handleGoogleSignIn = enhancedAuthHandler(async () => {
    ...
    try {
      ...
      console.log('[Google登录] 重定向到Google授权页面');
    } catch (error) {
      console.error('[Google登录] 发起Google登录失败:', error);
  ...
  return (
    <div className="flex flex-col w-full space-y-6">
      ...
      >
        ...
        {isGoogleLoading ? '正在连接...' : '使用Google账号登录'}
      </Button>
