...
async function processPaymentCallback(params: Record<string, any>) {
  ...
  try {
    ...
    return await transactionClient.executeTransaction(async (client) => {
      ...
      if (isSuccess || params.trade_no || params.transaction_id) {
        console.log("检测到支付可能成功，处理充值:", orderNo);
        ...
        // 更新回调日志状态
        await updateCallbackStatus(orderNo, 'failed', `支付失败，状态: ${params.trade_status || '未知'}`);
