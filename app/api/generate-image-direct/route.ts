  ...
  isProcessing = true;
    ...
    try {
      ...
      if (imageContent) {
        ...
        if (base64Image) {
          try {
            logger.info('开始将用户图片转换为URL格式');
            
            // 使用优化后的ensureImageUrl函数
            imageUrl = await ensureImageUrl(base64Image, user.id);
            
            if (imageUrl) {
              temporaryImageUrl = imageUrl; // 标记为临时URL
              logger.info('图片成功转换为URL格式');
              
              // 转换成功后清空base64数据，减少内存占用和日志干扰
              base64Image = null;
            } else {
              logger.warn('无法将图片转换为URL，将继续使用base64格式');
            }
          } catch (uploadError) {
            logger.warn(`图片转换失败: ${uploadError instanceof Error ? uploadError.message : String(uploadError)}`);
            // 使用安全摘要记录诊断信息，避免完整记录base64
            logger.debug('上传失败详情:', {
      ...
      // 阶段2: 使用gen_id处理用户图片
      if (genId && (base64Image || imageUrl)) {
        logger.info(`阶段2: 使用gen_id处理用户图片(${imageUrl ? 'URL模式' : 'base64模式'})`);
        
        try {
          const imageDataContent: any = {
            prompt,
          ...
          if (imageUrl) {
            imageDataContent.image_url = imageUrl;
            logger.info('在第二阶段使用图片URL传递图片数据');
          } else if (base64Image) {
            imageDataContent.image = base64Image;
            logger.info('在第二阶段使用base64传递图片数据');
            // 记录base64长度但不记录内容
            logger.debug(`base64数据长度: ${base64Image.length}字符`);
          }
          
          // 避免记录完整消息内容（可能包含大量base64），仅记录部分
          logger.debug(`第二阶段请求数据: ${JSON.stringify({
            ...imageDataContent,
            image: imageDataContent.image ? '[图片数据已省略]' : undefined,
          })}`);
