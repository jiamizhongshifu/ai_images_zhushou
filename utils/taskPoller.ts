  ...
  return new Promise((resolve, reject) => {
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        // 如果没有实际进度信息，则使用estimatedProgress或根据waitTime估算
        if (currentProgress === null || currentProgress === undefined) {
          currentProgress = result.estimatedProgress || getEstimatedProgress(result.waitTime || 0);
        }
        
        // 如果没有实际阶段信息，则使用processingStage或根据waitTime估算
        if (!currentStage) {
          currentStage = result.processingStage || getStageFromWaitTime(result.waitTime || 0);
        }
